    ...
    try {
        ...
        if(empty($body['vault_id']) && !empty($body['credit_card_type'])) {
            ...
            $card->setType($body['credit_card_type'])
                ...
                ->setBillingCountry($body['buyer_country_iso2'])
                ->setLastName($body['last_name']);                    
            $fi = new FundingInstrument();
            $fi->setPaymentCard($card);
            $payer->setPaymentMethod("credit_card")
                ->setFundingInstruments(array($fi));
        } else if (!empty($body['vault_id'])) {
        ...
        $price = $body['amount'];
        $item1 = new Item();
        $item1->setName(substr($body['name'], 0, 100))
            ->setDescription(substr($body['description'], 0, 100))
        ...
        $redirectUrls->setReturnUrl($body['success_url'])
